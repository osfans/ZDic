...
static int LZSS_Decoder(const unsigned char *src, long src_length, unsigned char *dst, long *dst_length);
...
static long read_bits(int bits, InputType *input);
...
// LZSS 解码器的主要解码副程式
static int LZSS_Decoder(const unsigned char *src, long src_length,
...
// None/LZSS/ZLib 
void Decoder(unsigned int zlibRefNum, int compressFlag, const unsigned char *src, long src_length,
	unsigned char *dst, long *dst_length)
{
	if(compressFlag == 0){
		...
		*dst_length=(uLongf)(CACHE_SIZE);		
		ZLibuncompress(zlibRefNum, (Bytef *)dst, (uLongf *)dst_length, (const Bytef *) src,(uLong) src_length);
		//ZLTeardown;
	}
}